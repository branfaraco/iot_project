"""Utility script to run the split‑grid first‑level LBCS processing.

This script loads metadata and mapping definitions from JSON files,
invokes the object-based split-grid processor from ``split_grid_processor``
and writes out both the simplified grid and a matrix of first-level codes
into a user-specified directory.

Adjust the ``base`` path below to point at your own ``BERLIN_reduced``
directory before running this script on your system.
"""
...
from split_grid_processor import run_split_processing_first_level


def main() -> None:
    """Run the first‑level LBCS processor and save the results.

    This function loads the metadata and mapping files, converts them
    into Python objects, executes the split‑grid processing pipeline
    and saves the resulting grid and matrix back to disk.  All I/O
    occurs in this function; the processing functions themselves only
    operate on in-memory objects and write cache files if needed.
    """
    # Base directory containing metadata.json and where output files
    # should be written.  Update this path to your environment.
