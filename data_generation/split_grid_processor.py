...
def _safe_query_recursive(bbox: Tuple[float, float, float, float], grid_type: str, cell_size: int, mapping: Dict[str, List[str]], cache_dir: str, level: int, max_recursion: int, attempt: int = 0) -> List[Dict[str, Any]]:
    ...
    grid_type : str
        Passed to `query_osm`. Either "h3" or "squares".
    cell_size : int
        Resolution or cell side length for `query_osm`.
    mapping : dict
        OSM tag→LBCS code mapping.
    cache_dir : str
        Directory for cached files.
    level : int
        Current recursion depth.
    max_recursion : int
        Maximum allowable recursion depth. Prevents infinite loops.
    attempt : int
        Internal counter tracking attempts for logging (unused
        currently but reserved for future heuristics).

    Returns
    -------
    list
        A list of mapped GeoJSON features from all subdivisions.
    """
    try:
